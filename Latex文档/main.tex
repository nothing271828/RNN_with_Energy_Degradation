...
\section{$RL^2$算法与PPO}
    假设每一次试验(Trial)包含包含M节任务(Epilsode)
    \subsection{环境交互与数据采集}
        对于每一次试验，时间步$t = 1, 2, \dots, N$：
        \begin{enumerate}
            \item \textbf{输入构建：}$x_t = \phi_{in}(s_t, a_{t-1}, r_{t-1}, d_{t-1})$，其中%
            $s_t$表示当前输入，$a_{t-1}$为上一步的行动，$r_{t-1}$为上一步的奖励，$d_{t-1}$为当前任务结束标记
            \item \textbf{RNN计算：}
            \begin{align}
                &h_t^p = f_{RNN}^p(x_t, h_{t-1}^p), \pi_\theta(a|h_t^p) = \phi_{out}^p(h_t^p)\\
                &h_t^v = f_{RNN}^v(x_t, h_{t-1}^v), V_\varphi(h_t) = \phi_{out}^v(h_t^v)\\
            \end{align}
            \item \textbf{环境交互:}
                动作采样$a_t \sim \pi_\theta(\cdot , h_t^p)$，然后与环境交互得到$r_t$, $s_{t+1}$, $d_t$
        \end{enumerate}

        在以上循环中，需要将每一步的$(x_t, h_t, a_t, r_t, \pi_\theta(a_t|h_t^p), V_\phi(h_t^v), d_t)$储存到Rollout Buffer
